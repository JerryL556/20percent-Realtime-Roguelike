...
## GitHub Pages
- Source: `main` branch, folder `docs` (GitHub Pages settings â†?Build and deployment â†?Source: Deploy from a branch â†?Branch: `main` / `/docs`).
...
## Systems Overview
- Game State & Saving
  - `src/core/GameState.js`: Tracks gold, HP, difficulty, dash charges, owned/equipped weapons, armour, and run seed. Handles scene progression (3 rooms â†?Boss â†?Hub) and provides difficulty modifiers.
...
## Scenes
- Boot â†?Start â†?Menu/Hub â†?Combat â†?Boss â†?Hub (loops). UI scene overlays gameplay scenes.
