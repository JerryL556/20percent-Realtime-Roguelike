...
export default class CombatScene extends Phaser.Scene {
  ...
  create() {
    ...
    this.physics.add.overlap(this.bullets, this.enemies, (b, e) => {
      ...
      // Rook shield: block non-rail bullets (including rockets) within 90Â° front arc
      if (e.isRook && !b._rail) {
        try {
    ...
    this.prompt = this.add.text(width / 2, 40, 'Clear enemies', { fontFamily: 'monospace', fontSize: 14, color: '#ffffff' }).setOrigin(0.5);// Init nav state for enemy pathfinding
  ...
  killEnemy(e) {
    ...
    try {
      if (!e.isDummy) {
        ...
        if (ui && ui.showResourceHint) ui.showResourceHint('+' + goldGain + ' Gold');
        const roll = Phaser.Math.Between(1, 100);
        const chance = isElite ? 50 : 10;
        if (roll <= chance) {
          this.gs.droneCores = (this.gs.droneCores || 0) + 1;
          if (ui && ui.showResourceHint) ui.showResourceHint('+1 Drone Core');
        }
      }
    } catch (_) {}
  ...
  update() {
    ...
    this.enemies.getChildren().forEach((e) => {
      ...
      if (!e.isSniper || (e.isSniper && !e.aiming)) {
        ...
        speed *= 1.5;
        // Rook: update and draw shield; turn slowly toward player (30Â°/s)
        if (e.isRook) {
          try {
